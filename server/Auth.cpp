...
std::string Auth::generateToken(int user_id, std::string username) {
    // Token format: base64(user_id:username).signature
    // where signature = SHA256(secret + base_part)
    
    std::string basePart = std::to_string(user_id) + ":" + username;
...
TokenPayload Auth::verifyToken(std::string token) {
    ...
    size_t colonPos = basePart.find(':');
    if (colonPos == std::string::npos) {
        return payload;
    }
    
    std::string userIdStr = basePart.substr(0, colonPos);
    std::string username = basePart.substr(colonPos + 1);
